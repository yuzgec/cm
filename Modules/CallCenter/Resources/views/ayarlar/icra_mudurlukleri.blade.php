@extends('master')
@section('title', 'Callcenter İcra Müdürlükleri | '.config('app.name'))
@section('content')
        ...
        <div class="card">
            <div class="card-body">
                <div class="mb-1 d-flex justify-content-between">
                    <div><h3>İcra Müdürlükleri</h3></div>
                    <div>
                    ...
                    <tbody>
                        @foreach($Liste as $row)
                            <tr>
                                ...
                                <td>{{$row->name}}</td>
                                <td>
                                    <a href="{{route('icra_mudurlugu.edit', $row->id)}}" class="btn btn-sm btn-warning">Düzenle</a>
    ...
    <div class="col-6">
        <div class="card">
            <div class="card-body">
                @if($item->id > 0)
                    {{Form::model($item, ["route" => ["icra_mudurlugu.update", $item->id]])}}
                    @method('put')
                @else
                    {{Form::model($item, ["route" => ["icra_mudurlugu.store"]])}}
                @endif
                    @csrf
