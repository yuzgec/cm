@extends('master')
@section('title', 'Callcenter Form Türleri | '.config('app.name'))
@section('content')
                    ...
                    <tbody>
                        @foreach($Liste as $row)
                            <tr>
                                ...
                                <td>{{$row->name}}</td>
                                <td>
                                    <a href="{{route('form_turleri.edit', $row->id)}}" class="btn btn-sm btn-warning">Düzenle</a>
    ...
    <div class="col-6">
        <div class="card">
            <div class="card-body">
                @if($item->id > 0)
                    {{Form::model($item, ["route" => ["form_turleri.update", $item->id]])}}
                    @method('put')
                @else
                    {{Form::model($item, ["route" => ["form_turleri.store"]])}}
                @endif
                    @csrf
